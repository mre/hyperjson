...
json_unicode = json.dumps if six.PY3 else functools.partial(
...
class UltraJSONTests(unittest.TestCase):
    ...
    def test_encodeStringConversion(self):
        ...
        def helper(expected_output, **encode_kwargs):
            output = json.dumps(input, **encode_kwargs)
    ...
    def testWriteEscapedString(self):
        self.assertEqual('"\\u003cimg src=\'\\u0026amp;\'\\/\\u003e"',
                         json.dumps("<img src='&amp;'/>", encode_html_chars=True))
    ...
    def testEncodeUnicodeBMP(self):
        ...
        if six.PY3:
            ...
            encoded = json.dumps(s, ensure_ascii=False).decode("utf-8")
    ...
    def test_encodeToUTF8(self):
        ...
        enc = json.dumps(input, ensure_ascii=False)
        dec = json.loads(enc)
    ...
    def test_decodeFromUnicode(self):
        input = "{\"obj\": 31337}"
        dec1 = json.loads(input)
        dec2 = json.loads(str(input))
    ...
    def test_encodeRecursionMax(self):
        ...
        self.assertRaises(OverflowError, json.dumps, input)
    ...
    def test_encodeOrderedDict(self):
        ...
        self.assertEqual('{"1": 1, "0": 0, "8": 8, "2": 2}', json.dumps(input))

    def test_decodeJibberish(self):
        input = "fdsa sda v9sa fdsa"
        self.assertRaises(ValueError, json.loads, input)
    ...
    def test_decodeArrayDepthTooBig(self):
        input = '[' * (1024 * 1024)
        self.assertRaises(RecursionError, json.loads, input)
    ...
    def test_decodeNumericIntPos(self):
        input = "31337"
        self.assertEqual(31337, json.loads(input))
    ...
    def test_decodeNullCharacter(self):
        input = "\"31337 \\u0000 31337\""
        self.assertEqual(json.loads(input), json.loads(input))
    ...
    def test_dumpFileArgsError(self):
        self.assertRaises(AttributeError, json.dump, [], '')

    def test_loadFile(self):
        f = six.StringIO("[1,2,3,4]")
        self.assertEqual([1, 2, 3, 4], json.load(f))
    ...
    def test_loadFileArgsError(self):
        self.assertRaises(AttributeError, json.load, "[]")

    def test_version(self):
        if six.PY2:
            self.assertRegexpMatches(
                json.__version__, r'^\d+\.\d+(\.\d+)?$', "json.__version__ must be a string like '1.4.0'")
        else:
            self.assertRegex(json.__version__, r'^\d+\.\d+(\.\d+)?$',
                             "json.__version__ must be a string like '1.4.0'")

    @unittest.skip("OverflowError not raised by dumps")
    def test_encodeNumericOverflow(self):
        self.assertRaises(OverflowError, json.dumps,
    ...
    def test_decodeNumberWith32bitSignBit(self):
        ...
        for doc, result in zip(docs, results):
            self.assertEqual(json.loads(doc)['id'], result)
    ...
    @unittest.skip("Not supported by json module, only by ujson and simplejson. See https://github.com/simplejson/simplejson/issues/52")
    ...
    def test_object_with_json_attribute_error(self):
        ...
        d = {u'key': JSONTest()}
        self.assertRaises(AttributeError, json.dumps, d)
    ...
    def test_decodeWithTrailingNonWhitespaces(self):
        input = "{}\n\t a"
        self.assertRaises(JSONDecodeError, json.loads, input)
    ...
    def test_decodeFloatingPointAdditionalTests(self):
        self.assertEqual(-1.1234567893, json.loads("-1.1234567893"))
        self.assertEqual(-1.234567893, json.loads("-1.234567893"))
        self.assertEqual(-1.34567893, json.loads("-1.34567893"))
        self.assertEqual(-1.4567893, json.loads("-1.4567893"))
        self.assertEqual(-1.567893, json.loads("-1.567893"))
        self.assertEqual(-1.67893, json.loads("-1.67893"))
        self.assertEqual(-1.7893, json.loads("-1.7893"))
        self.assertEqual(-1.893, json.loads("-1.893"))
        self.assertEqual(-1.3, json.loads("-1.3"))
    ...
    def test_ReadTrue(self):
        self.assertEqual(True, json.loads("true"))

    def test_ReadFalse(self):
        self.assertEqual(False, json.loads("false"))

    def test_ReadNull(self):
        self.assertEqual(None, json.loads("null"))

    def test_WriteTrue(self):
        self.assertEqual("true", json.dumps(True))

    def test_WriteFalse(self):
        self.assertEqual("false", json.dumps(False))
    ...
    def test_encodingInvalidUnicodeCharacter(self):
        ...
        """
        s = "\udc7f"
        self.assertRaises(UnicodeEncodeError, json.dumps, s)

    def test_sortKeys(self):
        data = {"a": 1, "c": 1, "b": 1, "e": 1, "f": 1, "d": 1}
        sortedKeys = json.dumps(data, sort_keys=True)
        self.assertEqual(
